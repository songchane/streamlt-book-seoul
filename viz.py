# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 15:15:39 2024
...
def numChart(total_df, sgg_nm):
    ...
    with col1:
        st.metric(label=f"아파트 거래 건수", value=result[result['HOUSE_TYPE'] == '아파트']['거래 건수'].values[0])
        st.metric(label=f"단독다가구 거래 건수", value=result[result['HOUSE_TYPE'] == '단독다가구']['거래 건수'].values[0])
    
    with col2:
        st.metric(label=f"오피스텔 거래 건수", value=result[result['HOUSE_TYPE'] == '오피스텔']['거래 건수'].values[0])
        st.metric(label=f"연립다세대 거래 건수", value=result[result['HOUSE_TYPE'] == '연립다세대']['거래 건수'].values[0])

def regionBarChart(total_df):
    st.markdown('## 지역별 평균 가격 막대 그래프')
    
    # 월 선택
    months = total_df['DEAL_YMD'].dt.month.unique()
    selected_month = st.selectbox('월을 선택하세요', months)
    
    # 가구 유형 선택
    house_types = total_df['HOUSE_TYPE'].unique()
    selected_house_type = st.selectbox('가구 유형을 선택하세요', house_types)
    
    # 필터링된 데이터프레임 생성
    filtered_df = total_df[(total_df['DEAL_YMD'].dt.month == selected_month) & 
                           (total_df['HOUSE_TYPE'] == selected_house_type)]
    ...
    fig = px.bar(x=mean_prices.index, y=mean_prices.values, 
                 labels={'x': '자치구명', 'y': '평균 가격'}, 
                 title=f'{selected_month}월 {selected_house_type} 지역별 평균 가격')
